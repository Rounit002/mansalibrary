...
import { Search, ChevronLeft, ChevronRight, Trash2, Eye, MessageCircle, ArrowUp, ArrowDown } from 'lucide-react';
...
interface Student {
  ...
  createdAt?: string;
  // Top-level seat number for easier access
  seatNumber?: string;
  assignments?: Array<{
    seatId: number;
    shiftId: number;
...
const ExpiredMemberships = () => {
  ...
  // State for sorting
  const [sortConfig, setSortConfig] = useState<{ key: keyof Student | null; direction: 'asc' | 'desc' }>({ key: null, direction: 'asc' });
  ...
  // Filtered and then sorted students
  const filteredAndSortedStudents = useMemo(() => {
    return sortedStudents.filter(
      (s) =>
        ...
        (s.assignments?.[0]?.seatNumber && s.assignments[0].seatNumber.toLowerCase().includes(searchTerm.toLowerCase()))
    );
          ...
          <div className="flex items-center space-x-4 mb-4">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search by name, phone, Reg. No., or Seat"
                value={searchTerm}
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="cursor-pointer" onClick={() => requestSort('registrationNumber')}>
                  ...
                  <TableHead className="cursor-pointer" onClick={() => requestSort('seatNumber')}>
                    Seat {getSortIcon('seatNumber')}
                  </TableHead>
                  <TableHead className="cursor-pointer" onClick={() => requestSort('membershipEnd')}>
                    Expiry {getSortIcon('membershipEnd')}
                  </TableHead>
